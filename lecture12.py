...
messages: List[BaseMessage] = [
    SystemMessage(f"""
   Ты - консультант по продажам магазина. 
   Тебе нужно отвечать на вопросы косательно возврата товара.
   Если пользователь спросил что-то, а у тебя по этому нет информации - скажи, что неможешь предоставить ответ на данный
   вопрос.
   #ПРАВИЛА ВОЗВРАТА
   {policy}
    """)
...
# якщо користувач просить перекласти речення, то дається переклад самого речення, а також пояснення
# граматики, наприклад структура there is\are, питання в різних часових формах, тощо.
...
# Для цього напишіть окрему модель яка буде діставати з відповіді(AIMessage) усі англійські слова(вважаємо що
...
# якщо користувач просить перекласти слово або фразу то дається переклад слова та приклад використання в реченні з вивченими словами
